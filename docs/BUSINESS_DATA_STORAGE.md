...
| 存储 | 内容 | 保留 |
| --- | --- | --- |
| PostgreSQL `app` | 运行/反馈摘要、政策同意、账号工作区、加密 MAA Box、公开排班历史、共享缓存与引用、第一方埋点明细 | 普通数据与埋点滚动 30 天；缓存 24 小时；最多 5 条固定排班长期保留 |
| `/var/lib` 私有目录 | CLI 输入输出、命令、stdout/stderr、调试包和反馈附件 | 7 天 |
| 浏览器 v5 | 白名单布局、Box、设置和最近排班；云同步元数据 | 30 天快速启动缓存 |

数据库不保存命令、路径、stdout/stderr、debug bundle、原始 CLI 请求响应或明文 MAA Box。森空岛 UID、昵称、Box、凭据和完整状态快照始终禁止入库；森空岛来源只允许同步不含身份的布局/设置，不保存来源名、排班历史，也不进入共享缓存。第一方埋点只允许保存隐私政策列明的稳定浏览器分析 ID、可选网站账号 ID、可选森空岛 HMAC、事件、页面、性能整数和设备类别，不保存 IP、完整 User-Agent 或事件请求正文。
...
`telemetry_event` 保存服务端接收的第一方体验明细并设置 30 天到期时间。每次埋点写入与既有业务维护都会删除过期行；网站用户外键级联删除，因此账号注销会删除账号关联事件，匿名事件只按到期时间清理。
...
撤销当前政策同意会删除工作区、版本、Box 密文、排班历史与该用户引用的全站缓存；共享缓存即使被其他用户使用也优先驱逐。网站账号删除依赖外键级联清除全部用户业务行和账号关联埋点，并在 Better Auth 删除成功后驱逐预先收集的缓存键。匿名埋点按 30 天 TTL 清理，私有文件继续按既有所有者标记和 7 天 TTL 清理。
